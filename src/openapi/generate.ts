  ...
  paths: {
    ...
    "/outlets": {
      ...
      get: {
        ...
        description: "Returns outlets deduplicated by outlet_id with nested campaign data. Each outlet appears once with a `campaigns` array containing per-campaign details. Filter by campaignId, brandId, status, featureSlug(s), or featureDynastySlug. Supports pagination on distinct outlets.",
        parameters: [
          ...
          { in: "query", name: "featureSlugs", schema: { type: "string" }, description: "Filter by feature slugs (comma-separated). Use a single slug or multiple." },
          { in: "query", name: "featureDynastySlug", schema: { type: "string" }, description: "Filter by feature dynasty slug (resolved to all versioned slugs via features-service). Takes priority over featureSlugs." },
          { in: "query", name: "limit", schema: { type: "integer", default: 100 }, description: "Max distinct outlets (default 100, max 1000)" },
          { in: "query", name: "offset", schema: { type: "integer", default: 0 }, description: "Pagination offset (on distinct outlets)" },
        ],
        responses: {
          "200": {
              ...
              "application/json": {
                schema: {
                  ...
                  properties: {
                    ...
                    total: { type: "integer", description: "Total number of distinct outlets matching filters" },
                  },
